...
class CASViewAdManager: NSObject {
    ...
    func showBannerAd(_ command: CDVInvokedUrlCommand) {
        let posIndex =
            command.arguments.first as? Int ?? Position.bottomCenter.rawValue
        let offsetX =
            command.arguments.count > 1
            ? (command.arguments[1] as? Int ?? 0) : 0
        let offsetY =
            command.arguments.count > 2
            ? (command.arguments[2] as? Int ?? 0) : 0
    ...
    private func refreshPosition() {
        guard let view = self.bannerView, let superview = view.superview else {
            return
        }
        ...
        case .topCenter, .topLeft, .topRight:
            ...
            if self.isPortraitSupported {
                y = view.topAnchor.constraint(
                    ...
                    constant: CGFloat(self.verticalOffset)
                )
            } else {
        ...
        case .topLeft, .bottomLeft, .middleLeft:
            x = view.leftAnchor.constraint(
                ...
                constant: CGFloat(self.horizontalOffset)
            )
